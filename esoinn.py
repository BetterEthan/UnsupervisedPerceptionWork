...
class ESoinn(BaseEstimator, ClusterMixin):
    ...
    def input_signal(self, signal: np.ndarray):
        ...
        # Algorithm 3.4 (3)
        winner, dists = self.__find_nearest_nodes(2, signal)  #返回winner的序号，和距离
        ...
        if dists[0] > sim_thresholds[0] or dists[1] > sim_thresholds[1]:
            ...
            # Algorithm 3.4 (4)
            self.__increment_edge_ages(winner[0])   #设定离得最近的winner所有邻居的年龄+1
            # Algorithm 3.4 (5)
            need_add_edge, need_combine = self.__need_add_edge(winner)  #判断是否需要连接两个winner
            if need_add_edge:
                # print("add edge")
                # Algorithm 3.4 (5)(a)
                self.__add_edge(winner)  # 需要连接，建立两个winner的连接
            else:
                # Algorithm 3.4 (5)(b)
                self.__remove_edge_from_adjacent_mat(winner)   #不需要连接，那么久删除连接
            # Algorithm 3.4 (5)(a) need combine subclasses
            if need_combine:
                self.__combine_subclass(winner)
            # Algorithm 3.4 (6) checked, maybe has problem
            self.__update_density(winner[0])  # 更新winner的密度
            # Algorithm 3.4 (7)(8)
            self.__update_winner(winner[0], signal) # add 1 to a local accumulated number of signals Ma1
            # Algorithm 3.4 (8)
            self.__update_adjacent_nodes(winner[0], signal)  #调整winner的weight vectors
    ...
    def __need_add_edge(self, winner):
        if self.node_labels[winner[0]] == self.INITIAL_LABEL or \
                        self.node_labels[winner[1]] == self.INITIAL_LABEL:  #某一个winner还没有定义类别
            return True, False  # 第一个决定是否连接， 第二个决定是否合并
        elif self.node_labels[winner[0]] == self.node_labels[winner[1]]: #两个winner属于同一类
            return True, False  # 连接
        else:  # 两个winner属于不同子类（公式7,8）
    ...
    def __find_nearest_nodes(self, num: int, signal: np.ndarray):
        n = self.nodes.shape[0]  #self.nodes 存放的节点
        indexes = [0] * num
    ...
    def __increment_edge_ages(self, winner_index):
        for k, v in self.adjacent_mat[winner_index, :].items():
            self.__set_edge_weight((winner_index, k[1]), v + 1)  #k[1] 应该带别的是序号， v+1为age
...
# print(len(pie))
    ...
    def findNearestCenter(self, signal):
        n = len(self.classCenter)  #self.nodes 存放的节点
        indexes = [0] * 1
