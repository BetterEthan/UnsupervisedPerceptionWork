...
class ContrastiveEncoder:
    ...
    def __init__(self, options):
        ...
        # print(self.contrastive_encoder)# output features 128 
        # input(444444444444444444444444)

    def set_contrastive_encoder(self):
        # Instantiate the model
        self.contrastive_encoder = self.get_encoder() # output features 128 
    ...
    def fit(self, data_loader):
        ...
        for epoch in range(self.options["epochs"]):
            ...
            for i, ((xi, xj), _) in self.train_tqdm:
                # plt.subplot(2, 1, 1)
                # img1 = xi[1,:,:,:].reshape(3,96,96)
                # img1 = img1.swapaxes(0,1)
                # img1 = img1.swapaxes(1,2)
                # print(img1.shape)
                # plt.imshow(img1)
                # plt.subplot(2, 1, 2)
                # img1 = xj[1,:,:,:].reshape(3,96,96)
                # img1 = img1.swapaxes(0,1)
                # img1 = img1.swapaxes(1,2)
                # plt.imshow(img1)
                # plt.show()
    ...
    def predict(self, train_loader):
        ...
        for i, ((Xbatch, _), Ybatch) in self.tqdm:
            ...
            # Forward pass on contrastive_encoder
            _, h = self.contrastive_encoder(Xbatch)  # z, h, z1 16,512,32
            # print(h.size())
