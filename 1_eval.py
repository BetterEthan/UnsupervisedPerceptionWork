...
def clustering(config):
    ...
    h_train, ytrain = model.predict(data_loader.train_loader)   # the middle layer output(h) is 512 dimension, the final layer is 128 dimension because of a nonlinear net
    ################# why h_train is 512 dimension
    ...
    # latentData = h_train
    # labelData = ytrain
    latentData = h_train[1:7000,:]
    labelData = ytrain[1:7000]
    ...
    import matplotlib.pyplot as plt
    X_tsne = TSNE(n_components=2, random_state=0).fit_transform(latentData)
    ...
    for label_ in labelData:
        ...
        plt.scatter(X_tsne[index_,0], X_tsne[index_,1], s=8, c=color[label_])
        index_ = index_ + 1
    plt.show()
    input('save or no..........')
    np.save('latentDataNew32',latentData)
    np.save('labelDataNew32',labelData)
